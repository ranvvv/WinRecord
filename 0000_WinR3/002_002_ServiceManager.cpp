...
static int WINAPI mWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
...
static void viewInit(HWND hwnd)
{
	...
	{
		sprintf(dest_file, "%s%s", path, "/dbg/*.sys");
		...
		{
			sprintf(dest_file, "%s/dbg/%s", path, pNextInfo.cFileName);
...
static void RemoveExtensionW(wchar_t* path)
{
	size_t i, len;
	len = lstrlenW(path);
	for (i = 0; i < len; i++)
